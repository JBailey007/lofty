const newAttainableFormHandler = async (event) => {
    ...
    const lofty_parent = document.URL[document.URL.length - 1]

    console.log(lofty_parent)
    // console.log(attainable_name)
    ...
    if (attainable_name && note) {
      const response = await fetch(`/api/attainable`, {
        method: 'POST',
        body: JSON.stringify({ attainable_name, note, lofty_parent}),
        headers: {
